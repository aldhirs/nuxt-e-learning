...
onMounted(async () => {
  if (session.value) {
    if (session.value.payment_method !== 'qris') {
  ...
  try {
    ...
    if (!snap.has_payment_session || snap.is_expired || snap.payment_method !== 'qris') {
      navigateTo(`/partner/invoice-pay/${invoiceNumber.value}`)
      return
    }
    paymentStore.setSession({
      ...
      qr_url: snap.qris_url,
      qr_string: snap.qris_string,
    })
  } catch {
    navigateTo(`/partner/invoice-pay/${invoiceNumber.value}`)
  }
...
const isExpiredQris = computed(() => false) // QRIS expiry detected server-side; no local check needed
const { showConfirmModal, isChanging, openConfirmModal, closeConfirmModal, confirmChange } = useChangePaymentMethod(
  ...
  { isPaid: computed(() => showSuccess.value), isExpired: isExpiredQris }
)
